...
Within the Edge Impulse platform, the trained model is exported to TensorFlow Lite format and quantized using Int8 PTQ, which applies per-tensor asymmetric quantization to activations and per-layer symmetric quantization to weights. The resulting model is further compiled by the EON Compiler, which statically resolves the computation graph, eliminating runtime interpreter overhead and further reducing RAM usage @novac2021quantization.

#figure(
  image("../../../../../../assets/figures/edge-impulse/008-platform-deployment-success-msg.png", width: 80%),
  caption: [Model Compilation and Build Success. The notification screen confirming the successful generation of the optimized C++ library for deployment on the Arduino UNO R4.],
) <fig-ei-compilation-success>

An Int8 TinyML fire detection model deployed under a comparable workflow achieved a validation accuracy reduction of only 1.36 percentage points after quantization, confirming the viability of Int8 deployment for safety-relevant classification tasks on microcontrollers @novac2021quantization. For the present system, Int8 is the default deployment format because it enables the model to reside comfortably within Flash and permits faster inference, while Float32 is retained as a reference baseline for validation.
