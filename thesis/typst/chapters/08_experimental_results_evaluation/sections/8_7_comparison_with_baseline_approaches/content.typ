...
Furthermore, the integration of frequency-domain features from the IR flame sensor provides an additional layer of verification for open-flame scenarios, which single-parameter gas sensors cannot provide. The comparative results demonstrate that the autonomous node maintains a detection accuracy exceeding 98% while reducing the false alarm rate to near zero across all tested scenarios, confirming that multi-modal fusion is essential for reliable fire safety at the edge @nekhil2023fire.
