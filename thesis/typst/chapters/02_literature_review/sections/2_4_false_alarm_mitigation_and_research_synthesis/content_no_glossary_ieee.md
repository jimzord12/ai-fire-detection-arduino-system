...
This thesis argues for the necessity of **three-class classification**: **fire, no_fire (ambient), and false_alarm**. This approach provides a more nuanced understanding of the environment, allowing the model to explicitly learn and recognize the unique signatures of common false alarm sources. The project goals (as outlined in `GEMINI.md`) specifically target this innovation to significantly reduce false positives by recognizing them as a distinct class. This explicit classification allows the system to:
